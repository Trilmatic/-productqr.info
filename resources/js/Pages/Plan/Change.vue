...
const not_subscribed = computed(() => {
    return props.plans.filter((el) => {
        return el.slug !== props.subscription.name;
    });
});
...
<template>
    ...
    <AppLayout>
        <section class="relative overflow-hidden p-8">
            <!-- Bg -->
            <div
                class="absolute inset-0 bg-primary pointer-events-none -z-10"
                aria-hidden="true"
            ></div>
                            ...
                            >
                                <form class="mb-4" @submit.prevent="">
                                    ...
                                    >
                                        ...
                                        <label
                                            :for="p.slug"
                                            class="py-4 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                                            >{{ __(p.slug) }} - €{{
                                                p.price
                                            }}</label
                                        >
